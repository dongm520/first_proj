...
def kakao():
    ...
    st.write("카카오톡 대화 내보내기 기능을 활용한 대화분석기입니다.")

    uploaded_file = st.file_uploader("TXT 파일 업로드", type=["txt"])
    if uploaded_file is None:
        st.write("카카오톡 '대화 내보내기'로 파일 생성")
        st.write("생성된 파일을 업로드")
        st.divider()
        st.text("화자 구분은 [      ]입니다. (예: [홍길동] : 나는 행복해.)")
...
- "reason"
 ...
 2. 설명에는 그 동물의 특징과 화자의 특징을 연결지어 설명할 것
